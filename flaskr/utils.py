...
from flask import current_app, g, flash, session
...
def check_trid(cookies) -> str:
    try:
        if 'trid' in cookies:
            trid = ''.join(str('%08x' % x) for x in struct.unpack('<4L', base64.b64decode(cookies['trid']))).upper()
            if 'trid' in session and session['trid'] == trid:
                pass  # sessionに保持している場合は送信済みのため送信しない
            else:
                sys_api = sw.SystemApi(sw.ApiClient(current_app.config.get('API_HOST')))
                tracking = sw.Tracking()
                tracking.tracking_id = trid
                sys_api.tracking_member_post(tracking=tracking)
                session['trid'] = trid
            return trid
