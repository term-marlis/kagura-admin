...
class ProjectUnSuccessList(AbstractParts):
    ...
    def columns(self):
        columns = [self.pt.c.project_id,
            ...
            text("IFNULL(DATEDIFF(STR_TO_DATE(pt.project_end_datetime, '%Y-%m-%d'), CURDATE()), 0) as remaining_days"),
    ...
    def where_project_public_status(self, form):
        if 'project_public_status' in form and form['project_public_status']:
            if form['project_public_status'] == '2':
                self.query = self.query.where(self.pt.c.project_public_status.in_([2, 3]))
            if form['project_public_status'] == '4':
                self.query = self.query.where(not self.pt.c.project_public_status.in_([2, 3]))
